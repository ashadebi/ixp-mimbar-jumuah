...
TOKEN=os.environ.get('SYSLOG_ANALYZER_TOKEN','').strip()
if len(TOKEN) < 24 or TOKEN in {'change-me','analyzer-secret-token','GENERATED_BY_install.sh'}:
 raise RuntimeError('SYSLOG_ANALYZER_TOKEN must be a strong explicit token')
DATA=os.environ.get('DATA_DIR','/data')
store=Store(os.path.join(DATA,'syslog.sqlite3'),retention=int(os.environ.get('RETENTION_SECONDS','129600')))
