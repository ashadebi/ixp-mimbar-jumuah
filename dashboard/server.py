...
def init():
    ...
    with connect() as c:
        ...
        CREATE TABLE IF NOT EXISTS stats_public_ports(port_id INTEGER PRIMARY KEY REFERENCES ports(id) ON DELETE CASCADE,enabled INTEGER NOT NULL DEFAULT 0,created REAL NOT NULL DEFAULT (strftime('%s','now')));
        CREATE TABLE IF NOT EXISTS stats_public_groups(id INTEGER PRIMARY KEY,name TEXT NOT NULL,mode TEXT NOT NULL DEFAULT 'selected',port_ids TEXT NOT NULL DEFAULT '[]',enabled INTEGER NOT NULL DEFAULT 1,created REAL NOT NULL DEFAULT (strftime('%s','now')));
        PRAGMA optimize;
        ''')
        try:
...
def stats_ports_rows(public_only=False):
    rows=query("""SELECT p.id AS port_id,p.switch_id,p.member_asn,p.port_number,p.type,p.status,p.bandwidth,p.mac_address,
                         s.name AS switch_name,n.name AS node_name,n.location,COALESCE(sp.enabled,0) AS public_enabled
                  FROM ports p JOIN switches s ON s.id=p.switch_id LEFT JOIN nodes n ON n.id=s.node_id
                  LEFT JOIN stats_public_ports sp ON sp.port_id=p.id
                  WHERE (?=0 OR COALESCE(sp.enabled,0)=1) ORDER BY n.location,s.name,p.port_number""",(1 if public_only else 0,))
    out=[]
    for r in rows:
        d=dict(r); sid=int(d['switch_id']); pid=int(d['port_id'])
        try:
            points=rrd_manager.fetch_rrd_data(sid,pid)
            known=[x for x in points if x.get('in_bps') is not None and x.get('out_bps') is not None]
            d['rrd_available']=bool(known)
            last=next((x for x in reversed(points) if x['timestamp']<=int(time.time())//300*300), {'in_bps':None,'out_bps':None})
            d['last_in_bps']=last['in_bps']; d['last_out_bps']=last['out_bps']
        except Exception:
            d['rrd_available']=False; d['last_in_bps']=d['last_out_bps']=None
        out.append(d)
    return out

def _stats_group_ports(group,public_only=True):
    enabled=stats_ports_rows(public_only)
    byid={int(x['port_id']):x for x in enabled}
    ids=list(byid) if group.get('mode')=='all' else [int(x) for x in group.get('port_ids',[]) if int(x) in byid]
    return [byid[x] for x in dict.fromkeys(ids)]

def get_stats_config(public_only=False):
    ports=stats_ports_rows(public_only)
    groups=[dict(g) for g in query('SELECT * FROM stats_public_groups WHERE (?=0 OR enabled=1) ORDER BY id',(1 if public_only else 0,))]
    for g in groups:
        try:g['port_ids']=json.loads(g.get('port_ids') or '[]')
        except Exception:g['port_ids']=[]
        selected=_stats_group_ports(g,public_only)
        g['aggregate']={'port_ids':[x['port_id'] for x in selected],
...
def stats_group_graph(group_id):
    ...
    if not g: raise Problem('Grup agregat tidak ditemukan.',404)
    g=dict(g)
    try:g['port_ids']=json.loads(g.get('port_ids') or '[]')
    except Exception:g['port_ids']=[]
    ports=_stats_group_ports(g,True)
    try:return rrd_manager.generate_aggregate_graph_png([(x['switch_id'],x['port_id']) for x in ports],title=g['name'])
    except FileNotFoundError: raise Problem('Data RRD agregat belum tersedia.',404)

def save_stats_config(d,actor='system'):
    ports=d.get('ports',[])
    if not isinstance(ports,list): raise Problem('Data port tidak valid.',400)
    valid={int(r['id']) for r in query('SELECT id FROM ports')}
    for item in ports:
        pid=int(item.get('port_id'))
        if pid not in valid: raise Problem('Port tidak ditemukan.',404)
        query('INSERT INTO stats_public_ports(port_id,enabled) VALUES(?,?) ON CONFLICT(port_id) DO UPDATE SET enabled=excluded.enabled',(pid,1 if item.get('enabled') else 0))
    query('DELETE FROM stats_public_groups')
    for g in d.get('groups',[]) if isinstance(d.get('groups',[]),list) else []:
        name=str(g.get('name','')).strip()[:80] or 'Agregat'
        mode=str(g.get('mode','selected'))
        if mode not in ('selected','all'): raise Problem('Mode agregat tidak valid.',400)
        raw=g.get('port_ids',[])
        if not isinstance(raw,list): raise Problem('Pilihan port agregat tidak valid.',400)
        ids=list(dict.fromkeys(int(x) for x in raw if int(x) in valid))
        query('INSERT INTO stats_public_groups(name,mode,port_ids,enabled) VALUES(?,?,?,?)',(name,mode,json.dumps(ids),1 if g.get('enabled',True) else 0))
    audit('stats','Pengaturan Stats/Graph diperbarui.',actor)
    return get_stats_config(False)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_request(self,method):
        try:
            ...
            if method=='GET' and path=='/api/public/stats':return self.response(get_stats_config(True))
            match=re.fullmatch(r'/api/public/stats/groups/(\d+)/graph',path)
            if method=='GET' and match:return self.response(stats_group_graph(int(match[1])),ctype='image/png')
            match=re.fullmatch(r'/api/public/stats/ports/(\d+)/(data|graph)',path)
            if match:
                pid=int(match[1]); kind=match[2]
                row=query('SELECT p.switch_id FROM ports p JOIN stats_public_ports sp ON sp.port_id=p.id AND sp.enabled=1 WHERE p.id=?',(pid,),one=True)
                if not row: raise Problem('Port tidak dipublikasikan.',404)
                if kind=='data': return self.response(rrd_manager.fetch_rrd_data(row['switch_id'],pid))
                return self.response(rrd_manager.generate_graph_png(row['switch_id'],pid),ctype='image/png')
            ...
            if method=='GET':
                ...
                if path=='/api/stats/config':return self.response(get_stats_config(False))
            ...
            if method=='POST' and path=='/api/stats/config':return self.response(save_stats_config(d,actor))
